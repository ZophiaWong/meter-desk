...
|---|---|---|---|---|
| PR backend quality checks | `.github/workflows/ci.yml` runs frozen Ruff check/format, pytest, and Markdown links; local checks are green; initial PR run `30679074517` failed in setup on nonexistent setup-uv `v8` alias, now corrected to verified `v8.3.2` | Actual successful corrected-head `backend-quality` GitHub job URL/result | P0-01 | Planned |
| PR frontend quality/build checks | `.github/workflows/ci.yml` runs `npm ci`, lint, typecheck, Vitest, and build; local checks are green; initial PR run `30679074517` succeeded on the pre-fix head | Actual successful corrected-head `frontend-quality` GitHub job URL/result | P0-01 | Planned |
| Automated Postgres integration | `.github/workflows/ci.yml` runs `make test-db` with unique project/port and unconditional cleanup; local checks are green; initial PR run `30679074517` failed in setup on the same action alias before checkout | Actual successful corrected-head `database-integration` GitHub job logs | P0-01 | Planned |
| CI container smoke | Dependency-gated `container-smoke` job runs with empty provider variables; initial PR run `30679074517` correctly skipped it after prerequisite failures | Actual successful corrected-head `container-smoke` GitHub job URL/result | P0-01 | Planned |
...
| Seeded full-stack runtime | `compose.yaml`, `Makefile`, and `scripts/container-smoke.sh` implement `postgres`, `migrate`, `seed`, `api`, `web` | Multiple unique-project `make container-smoke` runs, including current candidate project ending `538112`, verified `/health`, `/health/db`, tickets `TCK-1042`/`TCK-1137`, Web content, loopback ports, project cleanup, and default-volume preservation | P0-01 | Verified |
