...
| Explicit workflow completion semantics | `CaseWorkflow`, `WorkflowStatus`, `CaseWorkflowTransition`, and `AgentRun.workflow_id`; [`P0-03 spec`](../specs/hardening/p0-03-workflow-state-consistency.md) | `tests/test_workflows.py`, `tests/test_p0_03_workflow_commands.py`; Workbench workflow timeline and seeded cycle transitions | P0-03 | Existing |
| Idempotent Workflow start/retry | `start_or_replay_run` and `Idempotency-Key` route contract | Same-key replay, different-key active conflict, needs-retry same-cycle, and HTTP `201`/`200` contract tests | P0-03 | Existing |
| Atomic finalization | Repository `finalize_run` writes run output, final traces, approval, and Workflow transition in one transaction | In-memory command tests plus `tests/test_p0_03_postgres_atomicity.py` DML-ordinal and post-flush rollback evidence | P0-03 | Existing |
| Atomic approval/mutation/trace | `approve_and_execute` locks Workflow -> Approval and commits mutation trace with `mock_executed` | `tests/test_p0_03_postgres_atomicity.py` and `tests/test_p0_03_postgres_concurrency.py` cover rollback, lock waits, and serial-order races | P0-03 | Existing |
| Approval-write failure safety | Transaction rollback leaves approval, mutation, trace, and Workflow consistent | Fresh-session snapshots after every injected DML ordinal and `after_flush_postexec` failure | P0-03 | Existing |
| Fail-closed Workflow migration | `20260806_0009_p0_03_workflow_state_consistency.py` preflight/backfill and strict constraints | `tests/test_p0_03_migration_evidence.py` runs real 0008→0009 fixtures, unique temporary DBs, and contradiction rollback checks | P0-03 | Existing |
