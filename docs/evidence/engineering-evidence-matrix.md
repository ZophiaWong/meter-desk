...
| Explicit workflow completion semantics | `CaseWorkflow`, `WorkflowStatus`, `CaseWorkflowTransition`, and `AgentRun.workflow_id`; [`P0-03 spec`](../specs/hardening/p0-03-workflow-state-consistency.md) | `tests/test_workflows.py`, `tests/test_p0_03_workflow_commands.py` cover legal/illegal transitions, retry, terminal state, and no-action distinction | Workbench Workflow timeline and seeded cycle transitions | P0-03 | Implemented |
| Idempotent Workflow start/retry | `start_or_replay_run` and `Idempotency-Key` route contract | Same-key replay, different-key active conflict, and needs-retry same-cycle tests | API response `201`/`200` contract and retry entry point | P0-03 | Implemented |
| Atomic finalization | Repository `finalize_run` writes run output, final traces, approval, and Workflow transition in one transaction | In-memory all-or-nothing command tests; SQL failure-injection evidence still required | Postgres transaction boundary | P0-03 | Implemented |
| Atomic approval/mutation/trace | `approve_and_execute` locks Workflow -> Approval and commits mutation trace with `mock_executed` | In-memory command tests; real Postgres lock-wait and injected-failure evidence still required | Approval Queue and mutation audit | P0-03 | Implemented |
| Approval-write failure safety | Transaction rollback leaves approval, mutation, trace, and Workflow consistent | Failure injection at each write point | Persisted Postgres state and transition references | P0-03 | Implemented |
| Fail-closed Workflow migration | `20260806_0009_p0_03_workflow_state_consistency.py` preflight/backfill and strict constraints | Offline SQL generated; contradictory legacy fixture matrix still required | Alembic head `20260806_0009` | P0-03 | Implemented |
