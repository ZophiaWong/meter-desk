...
M5 hardens the Duplicate Charge golden path. It does not add new agent workflows, real integrations,
deployment, production monitoring, or mock-provider execution mode.
...
- No mock mutation exists before approval.
...
Live provider behavior remains unchanged. A real `POST /tickets/TCK-1042/agent-runs` still requires
provider configuration and still fails with `503` before creating a run when configuration is
missing.
...
- `make demo-reset-live` resets only `TCK-1042` runtime state.
...
After the reset, the Workbench returns to the no-run state and the operator can start the real M3
agent loop if provider configuration is present.
...
Eval Lab keeps supporting scenario results visible as `blocked` coverage gaps. Hiding these gaps
would weaken the eval story.
...
- Eval Lab behavior and why supporting cases are blocked.
...
- live reset clears only `TCK-1042` runtime state.
...
- frontend routes render the completed sample, reset/no-run state, provider failure state, approval
  states, and Eval Lab blocked gaps.
